...
module.exports = {
  ...
  "plugins": [
    ...
    ["@semantic-release/exec", {
      "prepareCmd": [
        "sed -i 's/@version.*/@version        ${nextRelease.version}/' ifonts-jenkins.user.css",
        // `|| true` keeps the release going when a hook rewrites a file and exits non-zero
        "pre-commit run --files CHANGELOG.md ifonts-jenkins.user.css || true"
      ].join(' && ')
    }],
    ["@semantic-release/git", {
      "assets": [ "CHANGELOG.md", "ifonts-jenkins.user.css" ],
